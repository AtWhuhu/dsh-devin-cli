...
function formatMessages(options) {
	...
	for (const message of options.messages) {
		...
		else if (block.type === "reasoning") parts.push(`[thinking]\n${block.text}`);
		else if (block.type === "tool-call") parts.push(`[tool-call ${block.id}: ${block.name}]\n${block.arguments}`);
		else if (block.type === "tool-result") {
			const texts = [];
			for (const sub of block.content) if (sub.type === "text") texts.push(sub.text);
	...
	*/
	...
	async *stream(options, signal) {
		...
		try {
			...
			sessionId = (await client.sessionNew(sessionParams, 15e3)).sessionId;
			let promptError;
			client.prompt(sessionId, toAcpPrompt(options)).then((response) => {
			...
			throw new LlmError(`Devin ACP stream failed (stderr tail: ${tail})`, "TRANSPORT", { cause: error });
		} finally {
			client.close();
		}
	}
...
//#endregion
export { ACP_PROTOCOL_VERSION, AcpStdioClient, Config, DevinAdapter, DevinBridgeServer, PROVIDER, apply, devinCredentialsPath, discoverDevinModels, inject, name, readDevinSession };
