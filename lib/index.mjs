	...
	async *stream(options, signal) {
		...
		try {
			...
			const settleActiveToolCalls = () => {
				if (dshSession && typeof dshSession.append === "function" && activeToolCalls.size > 0) for (const [toolCallId, active] of activeToolCalls) try {
			...
			const flushInitialBuffers = function* () {
			...
			while (true) {
				...
				if (update.sessionUpdate === "tool_call") {
					...
					if (dshSession && typeof dshSession.append === "function" && resolvedToolCallId) {
						...
						const isFinished = status === "completed" || status === "failed" || status === "success" || status === "done" || status === "finished" || status === "error" || tcu.isError !== void 0;
					...
					if (text) {
						if (!hasToolCalls && !bypassToolBuffering) initialThoughtBuffer += text;
						else if (hasToolCalls) {
							if (activeToolCalls.size > 0) settleActiveToolCalls();
							const { index, startChunk, endChunk } = ensureBlock("reasoning");
							if (endChunk) yield endChunk;
							if (startChunk) yield startChunk;
					...
					for (const content of contents) if (content.type === "text" && content.text) {
						if (!hasToolCalls && !bypassToolBuffering) {
							...
							if (initialTextBuffer.length > 100) yield* flushInitialBuffers();
						} else if (hasToolCalls) {
							if (activeToolCalls.size > 0) settleActiveToolCalls();
							const { index, startChunk, endChunk } = ensureBlock("text");
							if (endChunk) yield endChunk;
			...
			if (!hasToolCalls && !bypassToolBuffering) yield* flushInitialBuffers();
...
function installDevinRpc(ctx, options = {}) {
	...
	const ensureModels = async (bin, signal) => {
		if (cachedModels.length === 0) cachedModels.push(...await discoverDevinModels(bin, signal));
		return cachedModels;
	};
	...
	ctx.inject(["connection"], (connCtx) => {
		const dispose = connCtx.connection.rpc.intercept("/api", (endpoint) => endpoint === "devin-cli" || endpoint.startsWith(`devin-cli/`), (endpoint, payload, signal) => {
			const relative = endpoint === "devin-cli" ? "" : endpoint.slice(10);
...
//#endregion
export { ACP_PROTOCOL_VERSION, AcpStdioClient, Config, DevinAdapter, PROVIDER, apply, devinCredentialsPath, discoverDevinModels, inject, installDevinRpc, mapDevinToolNameToDsh, name, readDevinSession };
