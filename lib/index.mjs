		...
		try {
			...
			while (true) {
				...
				if (update.sessionUpdate === "tool_call") {
					...
					if (text) {
						if (hasToolCalls) postToolThoughtBuffer += text;
						else if (isPureChat) {
							const { index, startChunk, endChunk } = ensureBlock("reasoning");
							if (endChunk) yield endChunk;
							if (startChunk) yield startChunk;
						...
						} else initialThoughtBuffer += text;
					}
					...
					for (const content of contents) if (content.type === "text" && content.text) {
						if (hasToolCalls) postToolTextBuffer += content.text;
						else if (isPureChat) {
							const { index, startChunk, endChunk } = ensureBlock("text");
							if (endChunk) yield endChunk;
							if (startChunk) yield startChunk;
							...
							if (initialTextBuffer.length > 120) yield* flushInitialBuffers();
						}
					}
			...
			if (hasToolCalls) {
				...
				if (postToolTextBuffer) {
					...
					yield {
						type: "text-delta",
						index,
						text: postToolTextBuffer
					};
				}
			} else yield* flushInitialBuffers();
