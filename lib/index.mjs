...
function formatMessages(options) {
	const parts = [];
	if (options.system) {
		const systemPrompt = options.system.includes("dsh-ui") ? `${options.system}\n${DSH_UI_SPEC_RULE}` : options.system;
		parts.push(`[system]\n${systemPrompt}`);
	}
...
function installDevinRpc(ctx, options = {}) {
	...
	const registerOnConnection = (conn) => {
		try {
			console.log("[dsh-devin-cli] Registering RPC channel /devin-cli");
			return conn.rpc.handle("/devin-cli", (endpoint, payload, signal) => handleRpc(endpoint, payload, signal));
		} catch (err) {
			console.warn("[dsh-devin-cli] Failed to register RPC on connection:", err);
		}
	};
	const existingConn = ctx.connection;
	if (existingConn?.rpc?.handle) registerOnConnection(existingConn);
