...
*/
function deriveLocalTitle(options) {
	const messages = Array.isArray(options.messages) ? options.messages : [];
	let raw = "";
	for (const message of messages) {
		const blocks = Array.isArray(message?.content) ? message.content : [];
		for (const block of blocks) if (block && typeof block === "object" && block.type === "text" && typeof block.text === "string") raw += `${block.text}\n`;
	}
	let source = raw;
	...
	if (jsonStart !== -1) try {
		...
		if (Array.isArray(parsed)) {
			...
			for (const item of parsed) {
				...
				for (const block of blocks) if (block && typeof block === "object" && block.type === "text") {
	...
	const cleaned = source.replace(/```[\s\S]*?```/g, " ").replace(/[`*#>_~|[\]{}]/g, " ").replace(/\s+/g, " ").trim();
...
var DevinAdapter = class extends LlmAdapter {
	...
	*/
	...
	async *stream(options, signal) {
		...
		const dshSessionId = purpose === "compaction" ? "__aux__" : options.sessionId || "default";
