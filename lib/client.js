...
function apply(ctx) {
	const DevinSettingsSection = () => {
		...
		useEffect(() => {
			loadData();
			const timer = setInterval(() => void fetchStatus().then((s) => s && setStatus(s)), 5e3);
			return () => clearInterval(timer);
		}, []);
		const handleRefresh = async () => {
			setRefreshing(true);
			try {
				const json = await (await fetch(`${BRIDGE_API}/api/refresh`, { method: "POST" })).json();
		...
		const handleSave = async () => {
			setSaving(true);
			try {
				if ((await (await fetch(`${BRIDGE_API}/api/settings`, {
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ activeModelIds: [...activeIds] })
				})).json()).ok) showToast(`已成功保存 ${activeIds.size} 个激活模型！`);
			} catch (e) {
				showToast(`保存失败: ${e}`);
			} finally {
				setSaving(false);
		...
		const filteredModels = query ? baseList.filter((m) => m.id.toLowerCase().includes(query) || m.name?.toLowerCase().includes(query)) : baseList;
		return h("div", { style: {
			padding: "24px",
			maxWidth: "960px",
			margin: "0 auto",
